...
async function generateInvoicePdf(req, params) {
  ...
  const page = pdf.addPage([565, 377]); // landscape, matches the real paper receipt's proportions
  const bold = await pdf.embedFont(StandardFonts.HelveticaBold);
  const { width, height } = page.getSize();

  const logoImage = await pdf.embedPng(await loadAsset(req, "logo-full.png"));
  ...
  page.drawImage(logoImage, { x: 30, y: height - 32 - logoH, width: logoW, height: logoH });
  ...
  // Receipt number, red, just the sequential portion
  const seqMatch = invoice.invoice_number.match(/(\d+)$/);
  const receiptStr = seqMatch ? seqMatch[1] : invoice.invoice_number;
  const numSize = 20;
  const numW = bold.widthOfTextAtSize(receiptStr, numSize);
  page.drawText(receiptStr, { x: width - 40 - numW, y: height - 92, size: numSize, font: bold, color: RED });

  page.drawLine({ start: { x: 30, y: height - 108 }, end: { x: width - 30, y: height - 108 }, thickness: 1.5, color: BLACK });

  // Fields: المبلغ / الاسم / الفحص / تاريخ الفحص
  let y = height - 148;
  ...
  async function field(labelKey, value, showUnit) {
    const img = await drawArabicImage(`${fieldLabels[labelKey]} :`, { size: 32, widthPx: 280, heightPx: 48, align: "right" });
    const drawH = 17, drawW = (img.height ? (img.height / img.width) * drawH : drawH);
    const realDrawW = (img.width / img.height) * drawH;
    page.drawImage(img, { x: width - 40 - realDrawW, y: y - 4, width: realDrawW, height: drawH });

    if (showUnit) {
      const unitImg = await drawArabicImage(fieldLabels.unit, { size: 26, widthPx: 100, heightPx: 36, align: "right" });
    ...
    if (value) {
      page.drawText(String(value), { x: 40, y, size: 13, font: bold, color: BLACK });
    }
    y -= 40;
  }
  ...
  // Real stamp, footer zone, bottom-right
  const stampW = 100, stampH = (stampImage.height / stampImage.width) * stampW;
  page.drawImage(stampImage, { x: width - 40 - stampW, y: dividerY - stampH - 8, width: stampW, height: stampH });

  // Footer: real address + phone
  let fy = dividerY - 24;
  const addrImg = await drawArabicImage(
    "عيادة 353 - المركز الطبي 3 - شارع ابو داوود الظاهرى - المنطقة الحادية عشر - مدينة نصر",
    { size: 14, widthPx: 900, heightPx: 26, align: "right" }
  );
  ...
  const phoneImg = await drawArabicImage("ت : 15184 - 0128887187", { size: 14, widthPx: 400, heightPx: 26, align: "right" });
