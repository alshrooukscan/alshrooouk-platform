...
// Drive returns 100 files per call unless told otherwise, and this asked for
// no page size and followed no page token - so any folder holding more than
// 100 files was silently truncated to its first 100. One patient's raw DICOM
// folder held 326 and this returned 100 of them. It stayed invisible because
// the patient endpoint re-adds anything recorded in patient_files that the
// walk missed, so the count still looked right on screen while the listing
// underneath was wrong. Pages through to the end now.
...
// Re-parents a file from one folder to another without re-uploading its
// bytes. Used when the DICOM gateway lands a study in the unmatched
// quarantine folder first (it doesn't yet know which patient it belongs to)
// and staff resolve it afterwards - the file moves into the patient's real
// Raw_DICOM folder in one Drive API call instead of a second multi-hundred-MB
