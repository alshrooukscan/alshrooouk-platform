...
// Reorganized patients now have their files inside a per-visit subfolder
// (e.g. "2026-08-18__e8345e4c") instead of loose in the patient folder
// directly. This lists one level deep: loose files stay ungrouped
// (groupLabel null, for patients not yet reorganized), files inside a visit
// subfolder come back tagged with that subfolder's name so the UI can show
// them under a visit heading instead of a bare folder card.
export async function listFilesGrouped(folderId) {
  ...
  const nested = await Promise.all(
    subfolders.map(async (sf) => {
      ...
      return kids.filter((f) => f.mimeType !== FOLDER_MIME).map((f) => ({ ...f, groupLabel: sf.name }));
    })
  );
