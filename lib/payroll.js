...
export async function buildPayslip(employeeId, period) {
  ...
  return {
    ...
    overtimeIndicativeValue: Number(data.overtime_indicative_value || 0),
    overtimeDecision: data.overtime_decision || null,
    overtimeAwaitingDecision: !!data.overtime_awaiting_decision,
    days: [],
    basePay: Number(data.gross || 0),
    scanCommission: Number(data.scan_commission || 0),
    commissionDays: Number(data.commission_days || 0),
    reportBonus: Number(data.report_bonus || 0),
    reportsTotal: Number(data.reports_total || 0),
    reportsBeyondThreshold: Number(data.reports_beyond_threshold || 0),
    reportsOffShift: Number(data.reports_off_shift || 0),
    reportsQualifying: Number(data.reports_qualifying || 0),
    reportRate: Number(data.report_rate || 0),
    bonuses,
    deductions,
