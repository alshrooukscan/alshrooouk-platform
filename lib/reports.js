  ...
  payments_ledger: {
    ...
    async rows({ from, to }) {
      const pays = await fetchAll(() =>
        supabaseAdmin
          .from("visit_payments")
          .select("paid_at, amount, payment_method, collected_by_name, visits(exam_date, patients(name))")
          .gte("paid_at", from)
  ...
  cash_by_employee: {
    ...
    async rows({ from, to }) {
      const pays = await fetchAll(() =>
        supabaseAdmin
          .from("visit_payments")
          .select("paid_at, amount, payment_method, collected_by_name")
          .gte("paid_at", from)
          .lte("paid_at", `${to}T23:59:59`)
      );
      const byPerson = {};
      for (const p of pays) {
        const who = p.collected_by_name || "(not recorded)";
  ...
  stock_levels: {
    ...
    async rows() {
      const items = await fetchAll(() =>
        supabaseAdmin.from("stock_items").select("name, brand, category, qty_remaining, reorder_level, unit_price").order("brand").order("name")
      );
      return items.map((i) => {
        ...
        return {
          ...
          "Value on hand": Math.round(qty * money(i.unit_price)),
        };
      });
    },
  },
