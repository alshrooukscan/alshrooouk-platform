...
// Holding area for a study the DICOM gateway received but could not match to
// a visit (StudyInstanceUID, AccessionNumber and PatientID all missed). The
// file lives here, visible to staff via the unmatched-studies queue, until
// someone resolves it - moveFile() then re-parents it into the patient's real
// Raw_DICOM folder, so it never needs re-uploading from the clinic.
let unmatchedFolderCache = null;
export async function ensureUnmatchedQuarantineFolder() {
  if (unmatchedFolderCache) return unmatchedFolderCache;
  const folderId = await findOrCreateFolder("_Unmatched DICOM Studies", ROOT);
  unmatchedFolderCache = folderId;
  return folderId;
}
