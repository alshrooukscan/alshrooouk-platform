...
const FILE_TYPE_SUBFOLDER = { raw_data: "Raw_DICOM", report: "Reports", other: "Exports" };
...
// PatientID8_Date_TypeLabel.ext, collision-safe within the destination folder.
export async function buildStandardFileName(patientId, examDate, fileType, originalName) {
  const idShort = patientId.slice(0, 8);
  const dateStr = examDate || new Date().toISOString().slice(0, 10);
  const typeLabel = fileType === "raw_data" ? "Scan" : fileType === "report" ? "Report" : "Export";
  const ext = originalName.includes(".") ? originalName.split(".").pop() : "";
  const base = `${idShort}_${dateStr}_${typeLabel}`;
